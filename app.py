...
r_model_name = "Random Forest Regression Production"
r_model_version = 1
...
c_model_name = "LogisticRegression Production"
c_model_version = 1
...
if select == 'Predict':
    ...
    with tab1:
        if st.button("Predict Price"):
            ...
            st.success(f"Predicted Price: {r_prediction[0]}")

    with tab2:
        if st.button("Good Investment or Not"):
            c_prediction = c_model.predict([input])
            st.success(f"Investment: {c_prediction}")            
